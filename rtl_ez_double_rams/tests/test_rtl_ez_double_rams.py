...
def create_program(a, b):
    ...
    program.mmio_write(length, name="c0", offset=0x38)
...
def test_main():
    ...
    b = np.random.randint(m, size=(1,), dtype=np.int32)
    f = create_function()
    res = f(a, b)
    assert np.array_equal(res, a + b[0])
    print("Scale by a constant passed")
